...
* To send a zero the dataline is pulled low after 0.400탎
* To send a one the dataline is pulled low after 0.900탎
*/

#define ws2812_ctot	(((ws2812_cpuclk/1000)*1250)/1000000)
#define ws2812_t1	(((ws2812_cpuclk/1000)*400 )/1000000)		// floor
...
uint32_t* ws2812_port_set;
uint32_t* ws2812_port_clr;

void ws2812_init( void )
{
    ws2812_port_set = &(arch_ioport_pin_to_base(ws2812_pin)->OUTSET.reg);
    ws2812_port_clr = &(arch_ioport_pin_to_base(ws2812_pin)->OUTCLR.reg);
}
